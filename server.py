...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "When posting with add_topic or reply, end the text with ~~~~ so your "
        "username is recorded.\n\n"
        "ONE ISSUE PER THREAD — each distinct issue must be its own separate "
        "discussion thread. Call add_topic once per issue; never bundle multiple "
        "unrelated issues into a single add_topic call or reply. Do not include "
        "== section headers inside a comment or reply body.\n\n"
        "WIKITEXT FORMATTING — posts use MediaWiki markup, not Markdown:\n"
        "  Bold:         '''bold'''      (not **bold**)\n"
        "  Italic:       ''italic''      (not *italic*)\n"
        "  Bold+italic:  '''''both'''''\n"
        "  Sections:     == Heading ==, === Sub ===   (not # Heading)\n"
        "  Bullet list:  * item          (not - item)\n"
        "  Numbered:     # item\n"
        "  Wikilink:     [[Article name]] or [[Article|display text]]\n"
